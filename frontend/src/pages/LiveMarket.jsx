...
export default function LiveMarket() {
  ...
  const fetchIndices = async () => {
    ...
    try {
      ...
      setIndices([]);
      setError(e.response?.data?.detail || 'Could not fetch live Sensex/Nifty data from Yahoo Finance.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
      ...
      {/* ── Index Cards ── */}
      {error && (
        <motion.div variants={itemVariants} className="glass-card rounded-xl p-5 border border-danger/30 bg-danger/5">
          <p className="text-sm font-semibold text-danger">Live market data unavailable</p>
          <p className="text-xs text-textSecondary mt-1">{error}</p>
        </motion.div>
      )}
